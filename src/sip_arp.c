...
struct arpt_arp *arp_find_entry (__u32 ip)
{
	...
	for (i = 0; i < ARP_TABLE_SIZE; i++)					//在ARP表中查找IP匹配项
	{
		...
		else if (arp_table[i].ipaddr == ip && arp_table[i].status == ARP_ESTABLISHED)//没有超时,查看是否IP地址匹配, 并且状态为已经建立
		{
...
{
	struct arpt_arp *found = arp_find_entry (ip);	//根据IP查找ARP表项
	if (found)
	{																							//找到对应表项
...
{
	int i;
	struct arpt_arp *found = update_arp_entry (ip, ethaddr);//更新ARP表项
	if (!found)																		//更新不成功
	{																							//查找一个空白表项将映射对写入

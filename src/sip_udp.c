...
{
	struct udp_pcb *pcb = (struct udp_pcb *) malloc (sizeof (struct udp_pcb));//申请pcb变量
	if (pcb != NULL)
	{
...
int SIP_UDPConnect (struct udp_pcb *pcb, struct in_addr *ipaddr, __u16 port)
{
	struct udp_pcb *ipcb;
...
int SIP_UDPSendTo (struct net_device *dev, struct udp_pcb *pcb, struct skbuff *skb, struct in_addr *dst_ip, __u16 dst_port)
{

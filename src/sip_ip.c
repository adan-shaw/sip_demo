...
struct skbuff *sip_reassemble (struct skbuff *skb)
{
	...
	for (ipr_prev = ipr = ip_reass_list; ipr != NULL;)
	{
		...
		//分片是否输入此条链: 目的IP地址匹配, 源IP地址匹配, 分片的ID匹配
		if (ipr->iphdr.daddr == fraghdr->daddr && ipr->iphdr.saddr == fraghdr->saddr && ipr->iphdr.id == fraghdr->id)
		{
			found = 1;									//属于这条链
			break;
		}
	...
	{																//找到合适的分组链: 当前数据位于分片第一个, 分组链上的头部不是第一个分片
		if (((fraghdr->frag_off & 0x1FFF) == 0) && ((ipr->iphdr.frag_off & 0x1FFF) != 0))
		{
			memcpy (&ipr->iphdr, fraghdr, IPHDR_LEN);//更新重组中的IP头部结构
		}
	}
	//检查是否为最后一个分组
	if ((fraghdr->frag_off & htons (0x2000)) == 0)
	{																//没有更多分组
...
//将当前的数据放到重组链上,并更新状态
	...
	void *pos = NULL;
	__u32 length = 0;
	for (skb_prev = NULL, skb_cur = ipr->skb, length = 0, found = 0; skb_cur != NULL && !found; skb_prev = skb_cur, skb_cur = skb_cur->next)
	{
		if (skb_prev != NULL)					//不是第一个分片
		{
			//接收数据的偏移值位于前后两个之间
			if ((offset < FRAG_OFFSET (skb_cur->nh.iph)) && (offset > FRAG_OFFSET (skb_prev->nh.iph)))
			{
				...
				//前面的分片长度覆盖当前数据?
				if (FRAG_OFFSET (skb_prev->nh.iph) + FRAG_LENGTH (skb_prev->nh.iph) > FRAG_OFFSET (skb_cur->nh.iph))
				{
	...
	//重新计算重组链上的总数据长度
	for (skb_cur = ipr->skb, length = 0; skb_cur != NULL; skb_cur = skb_cur->next)
	{
		length += skb_cur->nh.iph->tot_len - IPHDR_LEN;
	}
	length += IPHDR_LEN;
	//全部的IP分片都已经接收到后进行数据报文的重新组合数据拷贝到一个新的数据结构中, 原来的数据接收都释放掉并从分组链中取出, 将重组后的数据结构指针返回
	if (length == ipr->datagram_len)//分组全部接收到?
	{
		...
		for (skb_prev = skb_cur = ipr->skb; skb_cur != NULL;)//遍历重组数据链
		{
			...
			memcpy (pos, skb_cur->tail, skb_cur->nh.iph->tot_len - skb_cur->nh.iph->ihl << 2);//将一个分片拷贝到新结构中
		}
...
int ip_input (struct net_device *dev, struct skbuff *skb)
{
	...
	case IPPROTO_ICMP:						//ICMP协议
		skb->th.icmph = (struct sip_icmphdr *) skb_put (skb, sizeof (struct sip_icmphdr));//ICMP头部指针获取
		icmp_input (dev, skb);			//转给ICMP模块处理
		break;
	case IPPROTO_UDP:							//UDP协议
		skb->th.udph = (struct sip_udphdr *) skb_put (skb, sizeof (struct sip_udphdr));//UDP头部指针获取
...
int ip_output (struct net_device *dev, struct skbuff *skb, struct in_addr *src, struct in_addr *dest, __u8 ttl, __u8 tos, __u8 proto)
{
